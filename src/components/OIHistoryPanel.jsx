...
export default function OIHistoryPanel({ selected, onClose }) {
  ...
  const [view, setView]       = useState('oi');   // 'oi' | 'price' — compare OI growth vs premium movement
  ...
  useEffect(() => { setView('oi'); }, [selected]);

  if (!selected) return null;

  const points = data?.points || [];
  const color = selected.type === 'CE' ? 'var(--gr)' : 'var(--rd)';
  const valKey = view === 'oi' ? 'oi' : 'ltp';
  const fmtVal = view === 'oi' ? fmtL : fmtP;

  // Compute change relative to previous point, for whichever field is active
  const pointsWithChange = points.map((p, i) => ({
    ...p,
    change: i > 0 ? p[valKey] - points[i-1][valKey] : null,
  }));

  // Sparkline for the active view
  let sparkline = null;
  if (points.length > 1) {
    const vals = points.map(p => p[valKey]).filter(v => v != null);
    if (vals.length > 1) {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:6, height:'100%', minHeight:0 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
        <span style={{ fontSize:9, color:'var(--mu)', letterSpacing:1, textTransform:'uppercase' }}>
          History — <span style={{ color }}>{selected.strike} {selected.type}</span>
        </span>
        <button onClick={onClose} style={{ fontSize:9, color:'var(--mu)', background:'none', border:'none', cursor:'pointer' }}>✕ back to movers</button>
      </div>

      {/* OI vs Price toggle — compare contract growth against premium movement */}
      <div style={{ display:'flex', gap:2, background:'var(--s2)', borderRadius:6, padding:2, border:'1px solid var(--bd)', flexShrink:0, width:'fit-content' }}>
        {[{key:'oi',label:'OI'},{key:'price',label:'PRICE'}].map(opt => (
          <button key={opt.key} onClick={()=>setView(opt.key)}
            style={{ fontSize:9, fontWeight:700, padding:'3px 10px', borderRadius:4, border:'none', cursor:'pointer',
              background: view===opt.key ? 'var(--cy)' : 'transparent',
              color:      view===opt.key ? '#000' : 'var(--mu)' }}>
            {opt.label}
          </button>
        ))}
      </div>
      ...
      {points.length > 0 && (
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'50px 1fr 70px', gap:6, fontSize:8, color:'var(--mu)', padding:'0 4px', flexShrink:0 }}>
            <span>Time</span><span>{view === 'oi' ? 'OI' : 'Premium'}</span><span style={{ textAlign:'right' }}>Change</span>
          </div>
          <div style={{ flex:1, minHeight:0, overflow:'auto', display:'flex', flexDirection:'column', gap:2 }}>
            {pointsWithChange.slice().reverse().map((p, i) => (
              <div key={i} style={{ display:'grid', gridTemplateColumns:'50px 1fr 70px', gap:6, alignItems:'center',
                ...
                <span style={{ fontFamily:'monospace', fontSize:10, color:'var(--tx)' }}>{fmtVal(p[valKey])}</span>
                <span style={{ fontFamily:'monospace', fontSize:9, textAlign:'right',
                  color: p.change > 0 ? 'var(--gr)' : p.change < 0 ? 'var(--rd)' : 'var(--mu)' }}>
                  {p.change != null ? (p.change > 0 ? '+' : '') + fmtVal(Math.abs(p.change)).replace('₹','') : '—'}
                </span>
              </div>
            ))}
          </div>

...
function usePriceDirection(scrips) {
  ...
  useEffect(() => {
    ...
    scrips.forEach(s => {
      ...
      if (prev != null && s.currentPrice != null) {
        next[s.symbol] = s.currentPrice > prev ? 'up' : s.currentPrice < prev ? 'down' : (directions[s.symbol] || 'flat');
      }
      if (s.currentPrice != null) prevRef.current[s.symbol] = s.currentPrice;
    });
...
function ScripTab({ scrip, active, direction, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{ display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:6,border:'none',
      ...
      {scrip.symbol}
      {scrip.currentPrice && (
        <span style={{ fontSize:10, color:priceColor, fontWeight:700, display:'flex', alignItems:'center', gap:2 }}>
  ...
  return (
    ...
    <div className="app-root" style={{ display:'flex', flexDirection:'column', height:'100vh', background:'var(--bg)', overflow:'auto' }}>
      <style>{`
        ...
        @media (max-width: 820px) { .ref-row { grid-template-columns: 1fr 1fr; } }
